...
abc2svg.Abc = function(w) {
    ...
    function Rg(a) {
                    ...
                    for (e = 0; e < n; e++, m -= 6)
                        if ("." != q[e]) {
                            ...
                            f -= 6) {
                                ...
                                if (window.total < window.count) window.total = window.count;
                            }
                            h += '"/>'
                        }
                    m = u[a].y;
        ...
        (function() {
            ...
            if (e) {
                ...
                var a_step = window.total;
                pre = window.b
                for (a = 0; a < e; a += a_step) 
                {
                    b = D[a];
                    var cc = pre, bb = Id(b[1]).toFixed(1) - b[2].toFixed(1)-30;
                    if ((Hd(b[0]).toFixed(1) - pre) < 10) continue;
                    pre = Hd(b[0]).toFixed(1);
                    window.num = window.num + 1;
                    // comment for displaying number
                    //z +='<text class="disptxt" x="'+cc+'" y="'+bb+'">'+num+'</text>\n';
    ...
    function Kc(a) {
        ...
        if (d = c.match(/-?[oO]blique/))
            b += "oblique ",
            c = c.replace(d[0], "");
        if (a.size.toFixed(1) == 10)
        {
            return "font:" + b + a.size.toFixed(1) + "px " + c + "; color : white"
        }
        else
        {
